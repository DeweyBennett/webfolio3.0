...
export default function Register({ session }) {
  ...
  const validationSchema = yup.object().shape({
    ...
    State: yup.string().max(2, 'Please use 2 digit state code').matches(/[\w][^script | img | onError | src][^<>="'?&$]/, { excludeEmptyString: true }),
    Zip_Code: yup.number().min(5).max(5),
    Password: yup.string().required().min(8, 'Password must be a minimum of 8 characters').max(12, 'Password must be a maximum of 12 characters').matches(/[\w][^script | img | onError | src][^<>="'?&$]/, { excludeEmptyString: true }),
  })
