...
export default function Register({ session }) {
  ...
  const validationSchema = yup.object().shape({
    First_Name: yup.string().required().matches(/[\w][^script | ^img | ^onError | ^src][^<>="'?&$]/, 'First name is invalid'),
    Last_Name: yup.string().required().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Last name is invalid'),
    Organization: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, { excludeEmptyString: true, 'Organization is invalid' }),
    Job_Title: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Job title is invalid'),
    Email: yup.string().required().email().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Email is invalid'),
    Phone_Number: yup.string().required().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Phone number is invalid'),
    Street_Address: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Street address is invalid'),
    City: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'City is invalid'),
    State: yup.string().max(2, 'Please use 2 digit state code').matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'State is invalid'),
    Zip_Code: yup.number().min(5).max(5),
    Password: yup.string().required().min(8, 'Password must be a minimum of 8 characters').max(12, 'Password must be a maximum of 12 characters').matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Password is invalid'),
  })
                ...
                <form onSubmit={handleSubmit(submitValidData)} className="">
                  ...
                  <div className="flex space-x-2">
                    ...
                    <div className="relative w-full mb-3">
                      ...
                      {errors.Job_Title &&
                      <div className="text-red-600 font-semibold">
                        <span>{errors.Job_Title.message}</span>
                      </div>
                      }
                    </div>
                  </div>
