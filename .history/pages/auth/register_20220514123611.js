...
export default function Register({ session }) {
  ...
  const validationSchema = yup.object().shape({
    First_Name: yup.string().required().matches(/[\w][^<>="'?&$]/),
    Last_Name: yup.string().required().matches(/[\w][^<>="'?&$]/),
    Organization: yup.string().matches(/[\w][^<>="'?&$]/, { excludeEmptyString: true }),
    Job_Title: yup.string().matches(/[\w][^<>="'?&$]/, { excludeEmptyString: true }),
    Email: yup.string().required().email().matches(/[\w][^<>="'?&$]/),
    Phone_Number: yup.string().required().matches(/[\w][^<>="'?&$]/,),
    Street_Address: yup.string().matches(/[\w][^<>="'?&$]/, { excludeEmptyString: true }),
    City: yup.string().matches(/[\w][^<>="'?&$]/, { excludeEmptyString: true }),
    State: yup.string().max(2).matches(/[\w][^<>="'?&$]/, { excludeEmptyString: true }),
    Zip_Code: yup.string().max(5).matches(/[\0-9][^<>="'?&$]/, { excludeEmptyString: true }),
    Password: yup.string().required().min(8).max(12).matches(/[\w][^script | img | onError | src][^<>="'?&$]/, { excludeEmptyString: true }),
  })
                ...
                <form onSubmit={handleSubmit(submitValidData)} className="">
                  ...
                  <div className="relative w-full mb-3">
                    ...
                    <input
                      ...
                      className="border-0 px-3 py-3 placeholder-blueGray-300 text-blueGray-600 bg-white rounded text-sm shadow focus:outline-none focus:ring w-full ease-linear transition-all duration-150"
                      {...register( 'Password' )}
                      title="Password can only contain uppsecase, lowercase, and the following special characters:!@*%#?"
                      value={passwordInput}
                      onChange={(e) => setpasswordInput(e.target.value)}
                    />
                    {errors.Password &&
                      <div className="text-red-600 font-semibold">
                        <span>Password can only contain uppsecase, lowercase, and the following special characters:!@*%#?</span>
                      </div>
                      }
                  </div>
                  ...
                  <div className="text-center mt-6">
                    <button
                      className="text-[#ffffff] px-2 py-4 w-[50%] mx-auto mt-4 rounded-full text-lg font-bold bg-gradient-to-r from-[#AE67FA] to-[#F49867] hover:bg-gradient-to-l hover:from-[#AE67FA] hover:to-[#F49867]"
                      type="submit"
                    >
                      Create Account
                    </button>
