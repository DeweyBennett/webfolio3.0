...
export default function Register({ session }) {
  ...
  const validationSchema = yup.object().shape({
    ...
    Job_Title: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
    Email: yup.string().required().email().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
    Phone_Number: yup.string().required().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
    Street_Address: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
    City: yup.string().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
    State: yup.string().max(2).matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
    Zip_Code: yup.number().min(5).max(5),
    Password: yup.string().required().min(8).max(12).matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'First name is invalid'),
  })
