...
export default function Login({ session }) {
  ...
  const validationSchema = yup.object().shape({
    Email: yup.string().required().email().matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Password is invalid'),
    Password: yup.string().required().min(8, 'Password is invalid').matches(/[\w][^script | img | onError | src][^<>="'?&$]/, 'Password is invalid')
  })

  const { register, handleSubmit, formState: { errors } } = useForm({ resolver: yupResolver(validationSchema) });
  console.log(errors)

  const submitValidData = (d) => {
