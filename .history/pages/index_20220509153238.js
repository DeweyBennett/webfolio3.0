...
export default function Home({ session, allBlogs, portProjects }) {
  ...
  const { data: PortProjects, error:PortProjectsLoadError } = useSWR('http://localhost:3000/api/blog', fetcher)
  console.log(PortProjects)
  if (BlogsLoadError || PortProjectsLoadError) return <div>failed to load</div>
  if (!Blogs || !PortProjects) return <div>loading...</div>
  ...
  return (
    <div className='bg-[#040C18] box-border m-0 p-0 scroll-smooth'>
      ...
      <div>
        <BlogSection blogs={Blogs.allBlogs} />
      </div>
      <Cta />
    </div>
  )
