...
const BlogFetcher = async () => {
  ...
  console.log("BlogFetcher data", data)
  return data
}
...
const PortfolioFetcher = async () => {
  ...
  const data = await res.json()
  console.log("PortfolioFetcher data", data)
  return data
}

const fetcher = async (url) => axios.get(url).then(res => res.json())
...
export default function Home({ session, allBlogs, portProjects }) {
  ...
  const { data: Blogs, error: BlogsLoadError } = useSWR('http://localhost:3000/api/blog', BlogFetcher)
  const { data: PortProjects, error: PortProjectsLoadError } = useSWR('http://localhost:3000/api/portfolio', PortfolioFetcher)

  console.log("Blogs and portfolio data", Blogs, PortProjects )
  console.log( BlogsLoadError, PortProjectsLoadError )
