...
export async function getServerSideProps(context) {
  ...
  if ( !session ) {
    ...
    const uUserRes = await fetch(`${process.env.BASE_URL}/api/auth/users`);
    const uUserData = await userRes.json();

    const rUserRes = await fetch(`${process.env.BASE_URL}/api/auth/users`);
    const rUserData = await userRes.json();
    ...
    return {
      props: {
        allBlogs: blogData,
        portfolios: portData,
        registeredUsers: rUserData.registered,
        unregisteredUsers: uUserData.unregistered,
