...
export default function Home({ session, allBlogs, portProjects }) {
  ...
  const { data: Blogs, error: BlogsLoadError } = useSWR('blogData', fetcher('http://localhost:3000/api/blog'))
  const { data: PortProjects, error: PortProjectsLoadError } = useSWR('http://localhost:3000/api/portfolio', fetcher('http://localhost:3000/api/portfolio'))

  console.log("Blogs and portfolio data", data )
  console.log( error )
