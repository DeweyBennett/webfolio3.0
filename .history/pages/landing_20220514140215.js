...
export default function Landing({ session }) {
  ...
  const validationSchema = yup.object().shape({
    ...
    Message: yup.string().required().matches(/[\w][^<>="'?&$]/)
  })
  ...
  const submitValidData = async (e) => {
    console.log(d)
    // const data = await axios.post(`/api/messaging/contactForm`, { firstName: firstNameInput, lastName: lastNameInput, email: emailInput, message: messageInput, role }, config).then(function (response) {
            ...
            <div className="flex flex-wrap justify-center lg:-mt-64 -mt-48">
              <div className="w-full lg:w-6/12 px-4">
                <div className="relative flex flex-col min-w-0 break-words w-full mb-6 shadow-lg rounded-lg bg-blueGray-200">
                  <form onSubmit={handleSubmit(submitValidData)} className="flex-auto p-5 lg:p-10">
